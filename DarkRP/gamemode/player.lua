...
function PlayerHit(ply, args)
	local t = ply:Team()
	local cost = GetGlobalInt("hitcost")
	if t == TEAM_HIT then
		Notify(ply, 1, 6, "Hitmen Cant Order Hits.")
	else
		local p = FindPlayer(args)
		if not ply:CanAfford(cost) then
			Notify(ply, 1, 6, "You Can Not afford to pay the Hitman")
		elseif p:Team() == TEAM_HITMAN then
			Notify(ply, 1, 6, "You cant order hits on Hitmen")
		elseif p:GetNWBool("hit") then
			Notify(ply, 1, 6, "This person already has a hit on him")
		elseif p and p:Alive() then
		p:SetNWBool("hit", true)
		ply:AddMoney(-cost)
			Notify(ply, 1, 6, "You have orderd a Hit on " .. p:Nick() )
		else
...
AddChatCommand("/hit", PlayerHit)

function HitDeath( victim, weapon, killer )
	local cost = GetGlobalInt("hitcost")
	if victim:GetNWBool("hit") then
		if killer:IsPlayer() then
			if killer:Team() == TEAM_HIT then
				if victim == killer then
					Notify(victim, 1, 4, "You can't collect hits on yourself")
				else
