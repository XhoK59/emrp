	...
	FirstTabPanel:EnableVerticalScrollbar( true )
		function FirstTabPanel:Update()
			...
			Commands:SetLabel("Actions")
				...
				local Hit = vgui.Create("DButton")
Hit:SetText("Hire a Hit")
Hit.DoClick = function()
local menu = DermaMenu()
for _,ply in pairs(player.GetAll()) do
if ply ~= LocalPlayer() and ply:Team() ~= TEAM_HIT then
menu:AddOption(ply:Nick(), function() LocalPlayer():ConCommand("say /hit " .. tostring(ply:UserID())) end)
end
end
if #menu.Panels == 0 then
menu:AddOption("No one can be hit!", function() end)
end
menu:Open()
end
ActionsPanel:AddItem(Hit)
...
function JobsTab()
	...
	function hordiv:Update()
		...
		local function AddIcon(Model, name, description, Weapons, command, special, specialcommand)
			if(name == "CHitman" )then
				name = "Hitman"
			end
			
			if(name == "CUnderCover Cop") then
				name = "Undercover Cop"
			end
